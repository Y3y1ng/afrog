const n=`内置函数源码位置：\`v2\\pkg\\runner\\celcompile.go\`

### brute（规则级枚举）

\`brute\` 不是 CEL 内置函数，而是 \`rules.<name>.brute\` 的规则能力：用于枚举一个或多个变量（payload），在同一条规则内自动发起多次请求并复用同一个 \`expression\`。
...
与 brute 相关的变量：
- brute payload 变量：例如上例的 \`p\`，可直接在模板里引用 \`{{p}}\`
- 截断标记：\`__brute_truncated_<rule>\`（bool），例如 \`__brute_truncated_r0\`
